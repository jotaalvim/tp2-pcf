...
waitList :: ListDur State -> Int ->  ListDur State
...
exec n s = LD (remLD (exec (n-1) s) ++ remLD (aux n s))
   
aux :: Int -> State -> ListDur State
aux 1 s = allValidPlays s
aux n s = do s1 <- allValidPlays s
             aux (n-1) s1
...
-- then we only have 7 minutes left, supposing each cross then only takes 1 minute (the fastest adventurer)
-- we only have 6 crossing lefts if we dont want to exceed the 7 minutes
